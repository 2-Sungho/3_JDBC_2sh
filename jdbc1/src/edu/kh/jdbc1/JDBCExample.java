...
public class JDBCExample {
	public static void main(String[] args) {
		// JDBC: java에서 DB에 접근할 수 있게 해주는 java programming API (java 기본 내장 인터페이스)
		// java.sql 패키지에서 제공
		
		/* *JDBC를 이용한 애플리케이션을 만들 때 필요한 것
		 * 1. java의 JDBC 관련 인터페이스
		 * 2. DBMS(Oracle)
		 * 3. Oracle에서 java 애플리케이션과 연결할 때
		 *    사용할 JDBC를 상속 받아 구현한 클래스모음 - ojdbc11.jar 라이브러리
		 *    -> OracleDriver.class(JDBC Driver) 이용
		 */
		...
		// Connection을 통해
		// SQL문을 DB에 전달하여 실행하고
		// 생성된 결과(ResultSet,성공한 행의 개수)를 반환(java)하는데 사용하는 객체
		...
		try {
			// 2단계: 참조변수에 알맞은 객체 대입
			// 1. DB 연결에 필요한 Oracle JDBC Driver 메모리에 로드하기 -> 객체로 만들어두기
			...
			// 3. SQL 작성
			// ** java에서 작성되는 SQL은 마지막에 ;세미콜론을 찍지 않아야한다! **
			...
			// execute(): SELECT문 수행 메서드, ResultSet 반환
