...
public class BoardView {
	...
	private void selectBoard() {
		...
		try {
			...
			Board board = bService.selectBoard(boardNo, MainView.loginMember.getMemberNo()); // 자신의 글 조회수 증가X
			if (board != null) {
				System.out.println("--------------------------------------------------------");
	 ...
	 * 
	 * @param board(상세 조회 된 게시글+작성자 번호+댓글 목록)
	 */
	...
	private void insertComment(int boardNo, int memberNo) {
		try {
			...
			comment.setCommentCotent(content);
