...
// 코드의 통일성을 지키기 위해서 Service에서 Connection 객체를 작성한다
...
public class TestService {
	...
	public int insert(TestVO vo1, TestVO vo2, TestVO vo3) throws Exception {
		...
		try {
			...
			// 트랜잭션 제어
			if(result1+result2+result3==3) { // 모두 insert성공한 경우
				commit(conn);
				res=1;
			} else {
				rollback(conn);
	...
	public int update(TestVO vo2) throws SQLException {
		Connection conn=getConnection();
		
		int result=dao.update(conn,vo2);
		if(result>0) commit(conn);
