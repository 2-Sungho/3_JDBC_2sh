...
-- 사용자 계정 생성
CREATE USER member_lsh IDENTIFIED BY member1234;

--생성한 사용자계정에 권한 부여
GRANT CONNECT, RESOURCE, CREATE VIEW TO member_lsh;

-- 테이블 스페이스 할당
ALTER USER member_lsh DEFAULT
...
INSERT INTO "MEMBER" VALUES (SEQ_MEMBER_NO.NEXTVAL,'user03','pass03','유저삼','M',DEFAULT,DEFAULT);
...
AND SECESSION_FL='N';
--> ID가 user01이면서 탈퇴하지 않는 회원 조회
-- 중복이면 1, 아니면 0 조회

-- 로그인(아이디,비밀번호일치 + 탈퇴X 회원)
...
-- 게시판 번호 시퀀스
CREATE SEQUENCE SEQ_BOARD_NO2 NOCACHE;
-- 게시판 샘플 데이터
SELECT * FROM "MEMBER";
SELECT * FROM BOARD ;
INSERT INTO BOARD
VALUES (SEQ_BOARD_NO2.NEXTVAL, '샘플제목1', '샘플내용1입니다.', TO_DATE('2022-09-20 10:30:12','YYYY-MM-DD HH24:MI:SS'),DEFAULT,DEFAULT,3);
INSERT INTO BOARD
VALUES (SEQ_BOARD_NO2.NEXTVAL, '샘플제목2', '샘플내용2입니다.', TO_DATE('2022-09-21 11:33:32','YYYY-MM-DD HH24:MI:SS'),DEFAULT,DEFAULT,3);
INSERT INTO BOARD
...
COMMENT ON COLUMN "COMMENT".MEMBER_NO IS '작성자 회원 번호';
...
-- 게시글 상세 조회(게시글 내용 조회)
SELECT BOARD_NO, BOARD_TITLE, BOARD_CONTENT, MEMBER_NO, MEMBER_NM, READ_COUNT, TO_CHAR(CREATE_DT,'YYYY-MM-DD HH24:MI:SS') CREATE_DT
FROM BOARD
JOIN "MEMBER" USING (MEMBER_NO)
WHERE DELETE_FL='N'
AND BOARD_NO=3;

-- 특정 게시글의 댓글 목록 조회(작성일 오름차순(오래된))
SELECT COMMENT_NO,COMMENT_CONTENT,MEMBER_NO,MEMBER_NM, TO_CHAR(CREATE_DT,'YYYY-MM-DD HH24:MI:SS') CREATE_DT
FROM "COMMENT"
JOIN "MEMBER" USING (MEMBER_NO)
WHERE DELETE_FL='N'
AND BOARD_NO=3
...
--AND (BOARD_TITLE LIKE '%'||?||'%' OR BOARD_CONTENT LIKE '%'||?||'%') -- 제목+내용
